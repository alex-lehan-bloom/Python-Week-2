...
class FileHandler:
    ...
    def append_to_csv(self, file_name, field_names, data_to_add_to_csv):
        try:
            ...
            user_id = self.get_number_of_rows_in_csv(file_name) + 1
            try:
    ...
    def get_number_of_rows_in_csv(self, file_name):
        try:
            csv_file = open(file_name, "r", newline='')
        except FileNotFoundError:
            print("Error: File not found: '{}'.".format(file_name))
...
my_file_handler.get_number_of_rows_in_csv("csv_files/User.csv")
